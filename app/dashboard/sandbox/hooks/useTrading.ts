...
export function useTrading(initialState: SandboxState) {
  ...
  const [historique, setHistorique] = useState<Transaction[]>(
    initialState.historique,
  );

  const passerOrdre = useCallback(
    (type: OrderType, stock: Stock, quantite: number) => {
      ...
      if (type === "achat") {
        ...
        setPositions((prev) => {
          ...
          if (existing) {
            ...
            const newAvg =
              (existing.avgBuyPrice * existing.quantity + prix * quantite) /
              newQty;
            return {
              ...prev,
              [nom]: {
                ...existing,
                quantity: newQty,
                avgBuyPrice: +newAvg.toFixed(4),
              },
            };
          }
          return {
            ...prev,
            [nom]: { stockName: nom, quantity: quantite, avgBuyPrice: prix },
          };
        });
      } else {
