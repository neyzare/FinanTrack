                    ...
                    <>
                        <div className="hidden md:block overflow-x-auto">
                            <table className="w-full text-sm min-w-[800px]">
                                ...
                                <tr className="border-t-2 font-bold">
                                    ...
                                    <td className="px-6 py-3 text-right tabular-nums">{totalQuantity}</td>
                                    <td colSpan={2} />
                                    <td className="px-6 py-3 text-right tabular-nums">
                                        {formatEur(valeurActions)} €
                                    </td>
                                    <td className="px-6 py-3 text-right">
                                        <div className={`flex flex-col items-end tabular-nums ${totalProfit ? 'text-green-500' : 'text-red-500'}`}>
                                            <span>{totalProfit ? '+' : ''}{formatEur(totalPl)} €</span>
                                            <span className="text-xs">{totalProfit ? '+' : ''}{totalPlPct.toFixed(2)}%</span>
                                        </div>
                                    </td>
                                    <td />
                                </tr>
                        ...
                        <div className="md:hidden p-4 space-y-3">
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-2">
                                            ...
                                            <div className={`flex flex-col items-end tabular-nums font-medium ${isProfit ? 'text-green-500' : 'text-red-500'}`}>
                                                <span>{isProfit ? '+' : ''}{formatEur(pl)} €</span>
                                                <span className="text-xs">{isProfit ? '+' : ''}{plPercent.toFixed(2)}%</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-3 space-y-2 text-sm">
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="tabular-nums">{formatEur(pos.avgBuyPrice)} €</span>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <span className="text-muted-foreground">Prix actuel</span>
                                                <span className="tabular-nums">{formatEur(currentStock.price)} €</span>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <span className="text-muted-foreground">Valeur</span>
                                                <span className="tabular-nums font-medium">{formatEur(currentValue)} €</span>
                                            </div>
                                        </div>
                            ...
                            <div className="rounded-lg border-2 border-border p-4 text-sm font-bold space-y-2">
                                ...
                                <div className="flex items-center justify-between">
                                    <span>Valeur</span>
                                    <span className="tabular-nums">{formatEur(valeurActions)} €</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span>P/L</span>
                                    <span className={`tabular-nums ${totalProfit ? 'text-green-500' : 'text-red-500'}`}>
                                        {totalProfit ? '+' : ''}{formatEur(totalPl)} € ({totalProfit ? '+' : ''}{totalPlPct.toFixed(2)}%)
                                    </span>
                                </div>
                            </div>
                        </div>
