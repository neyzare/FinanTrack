...
export function MarketTable({ stocks, positions, liquidite, nameSelectionne, onSelectStock, onOrder }: MarketTableProps) {
    return (
        <Card>
            <CardHeader>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-end gap-2">
                                            <Button
                                                ...
                                                onClick={(e) => { e.stopPropagation(); onOrder(stock, "achat"); }}
                                            >
                                                Acheter
                                            </Button>
                                            <Button
                                                ...
                                                variant="outline"
                                                disabled={held === 0}
                                                onClick={(e) => { e.stopPropagation(); onOrder(stock, "vente"); }}
                                            >
                                                Vendre
                                            </Button>
                                        </div>
                        ...
                        return (
                            <div
                                ...
                                className={`rounded-lg border p-4 cursor-pointer transition-colors ${
                                    estSelectionne ? 'border-primary bg-primary/5' : 'border-border'
                                }`}
                            >
                                ...
                                <div className="mt-2 flex items-center justify-between">
                                    <span className="text-lg font-medium tabular-nums">{formatEur(stock.price)}€</span>
                                    <span className={`flex items-center gap-1 text-sm font-medium tabular-nums ${hausse ? 'text-green-500' : 'text-red-500'}`}>
                                        {hausse ? <TrendingUp className="w-3.5 h-3.5" /> : <TrendingDown className="w-3.5 h-3.5" />}
                                        {hausse ? '+' : ''}{stock.variation.toFixed(2)}%
                                    </span>
                                </div>
