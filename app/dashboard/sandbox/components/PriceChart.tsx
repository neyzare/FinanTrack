...
interface CandlestickShapeProps extends Pick<
  BougieChart,
  "open" | "high" | "low" | "close"
> {
  x?: number;
  ...
  return (
    <div className="rounded-lg border bg-card p-3 shadow-md text-sm space-y-1">
      <p className="text-muted-foreground text-xs">
        {formatTimeLabel(time, tf)}
      </p>
      <p>
        O : <b>{open.toFixed(2)}</b> &nbsp; H :{" "}
        <b className="text-green-500">{high.toFixed(2)}</b>
      </p>
      <p>
        L : <b className="text-red-500">{low.toFixed(2)}</b> &nbsp; C :{" "}
        <b style={{ color }}>{close.toFixed(2)}</b>
      </p>
    </div>
  );
}
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              <span
                className={`text-sm font-medium ${stockSelectionne.variation >= 0 ? "text-green-500" : "text-red-500"}`}
              >
                {stockSelectionne.variation >= 0 ? "+" : ""}
                {stockSelectionne.variation.toFixed(2)}%
              </span>
            </CardTitle>
            <p className="text-2xl font-bold mt-1">
              {stockSelectionne.price.toLocaleString("fr-FR", {
          ...
          >
            ...
            >
              ...
              <Bar
                ...
                shape={(props) => (
                  <CandlestickShape
                    {...(props as unknown as CandlestickShapeProps)}
                  />
                )}
              >
                <ErrorBar
