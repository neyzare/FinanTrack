...
import {
  ...
  SEVERITY_LABELS,
  type Anomaly,
  type AnomalyReport as Report,
} from "@/app/types/anomaly";
    ...
    <li className="border-t pt-4 first:border-t-0 first:pt-0">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Badge
            variant="outline"
            className={severityStyles[anomaly.severity] ?? "text-foreground"}
          >
            sévérité {severity}
          </Badge>
    ...
    <Card className="border-2">
      <CardHeader>
        <CardTitle className="font-semibold">
          Détection d&apos;anomalies — {report.symbol}
        </CardTitle>
        <CardDescription>
      ...
      <CardContent className="space-y-6">
        ...
        {report.anomalies.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Aucune anomalie au-dessus du seuil sur la période : le titre est
            resté dans son comportement statistique habituel.
          </p>
        ) : (
          <ul className="space-y-4">
            {report.anomalies.map((anomaly) => (
              <AnomalyRow
                key={`${anomaly.date}-${anomaly.composite_score}`}
                anomaly={anomaly}
              />
            ))}
          </ul>
