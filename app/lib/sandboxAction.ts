...
export async function getSandboxState(): Promise<SandboxState> {
    ...
    const row = await prisma.sandboxState.findUnique({ where: { userId: user.id } });
...
export async function placeOrder(
    ...
    try {
        ...
        const stock = await prisma.sandboxStock.findFirst({ where: { name: stockName } });
        ...
        if (type === "achat") {
            if (liquidite < total) return { success: false, error: "Liquidité insuffisante" };
            liquidite = +(liquidite - total).toFixed(2);

            if (index >= 0) {
                const pos = positions[index];
                const newQuantity = pos.quantity + quantity;
                const newAverage = (pos.avgBuyPrice * pos.quantity + unitPrice * quantity) / newQuantity;
                positions = positions.map((p, i) =>
                    i === index
                        ? { ...p, quantity: newQuantity, avgBuyPrice: +newAverage.toFixed(4) }
                        : p,
                );
            } else {
                positions = [...positions, { stockName, quantity, avgBuyPrice: unitPrice }];
            }
        } else {
            const held = index >= 0 ? positions[index].quantity : 0;
            if (quantity > held) return { success: false, error: "Quantité détenue insuffisante" };

            liquidite = +(liquidite + total).toFixed(2);
            const remaining = held - quantity;
            positions = remaining <= 0
                ? positions.filter((_, i) => i !== index)
                : positions.map((p, i) => (i === index ? { ...p, quantity: remaining } : p));
        }
