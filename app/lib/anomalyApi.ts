...
// Jamais NEXT_PUBLIC_ : ce préfixe inlinerait la clé dans le bundle envoyé
// au navigateur. Ce module est "use server", la clé reste sur le serveur.
const API_KEY = process.env.ANOMALY_API_KEY;
...
export async function getAnomalies(
  ...
  if (!API_URL || !API_KEY) {
    return {
      success: false,
      error: "ANOMALY_API_URL ou ANOMALY_API_KEY n'est pas configurée.",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return {
          success: false,
          error: `Aucune donnée de marché pour ${ticker}.`,
        };
      }
      if (response.status === 401 || response.status === 403) {
        // Problème de configuration, pas d'utilisation : on le trace côté
        // serveur sans distinguer « clé absente » de « clé invalide » pour
        // l'utilisateur — cette nuance ne l'aide pas et renseigne un attaquant.
        console.error("[anomalyApi] authentification refusée", response.status);
        return { success: false, error: "Service d'analyse indisponible." };
      }
      if (response.status === 429) {
        ...
        return {
          success: false,
          error: retryAfter
            ? `Trop de requêtes d'analyse. Réessayez dans ${retryAfter} s.`
            : "Trop de requêtes d'analyse. Réessayez dans un instant.",
        };
      }
      if (response.status === 504) {
        return {
          success: false,
          error: "Le fournisseur de données de marché n'a pas répondu.",
        };
      }
      if (response.status === 422) {
        return {
          success: false,
          error: "Paramètres d'analyse refusés par l'API.",
        };
      }
      return {
        success: false,
        error: `L'API d'analyse a répondu ${response.status}.`,
      };
    }
    ...
    if (!parsed.success) {
      ...
      return {
        success: false,
        error: "Réponse inattendue de l'API d'analyse.",
      };
    }
